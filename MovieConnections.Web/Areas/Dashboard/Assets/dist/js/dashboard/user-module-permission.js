$.getScript('/areas/dashboard/assets/vendor/kendo.ui/js/kendo.aspnetmvc.min.js', function () {
    ...
    var toolbar = [{
        name: "select-user",
        template: "<input id=\"UserList\" class=\"form-control input-sm\" style=\"width:280px;\"/>"
    }];
    ...
    var grid = $('#grid').kendoExtendedGrid({
        dataSource: {            
            transport: {
                read: {
                    url: "/Dashboard/UserModulePermission/List",
                    type: "post"
                }
            },
            schema: {
    ...
    var userList = $("#UserList").kendoComboBox({
        ...
        dataSource: {
            transport: {
                read: {
                    url: "/Dashboard/UserModulePermission/GetUsers",
                    type: "POST"
                }
            }            
        },
    ...
    $("#saveChanges").on("click", function () {
        $.ajax({
            type: "POST",
            url: "/Dashboard/UserModulePermission/CreateOrUpdate",
            data: {
                items: JSON.stringify(grid.dataSource.data()),
                userId: $("#UserList").data("kendoComboBox").value()
            },
            async: false,
            success: function () {
